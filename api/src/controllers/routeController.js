const services = require('../services/routeServices');
...
const getInactive = async (req, res) => {
  try {
    const routes = await services.getInactive();
    res.status(200).json(routes).end();
  } catch (err) {
    res.status(500).json(err).end();
  }
...
const getById = async (req, res) => {
  try {
    ...
    // 404 not found. significa que o servidor não encontrou uma rota com o id especificado
...
const create = async (req, res) => {
  try {
    ...
    const route = await services.create(req.body);
    // o código HTTP 201 inidica que um conteúdo foi criado com sucesso no servidor
    res.status(201).send(`Route added with Route : ${route.name}`).end();
  } catch (err) {
    ...
    res.status(422).send(`cannot create route: ${err}`).end();
  }
};
...
const update = async (req, res) => {
  try {
    ...
    if (id) {
      ...
      res
        .status(200)
        .json('Rota atualizado com sucesso.' + qtd)
        .end();
    } else {
      // 400 Bad Request Essa resposta significa que o servidor não entendeu a requisição pois está com uma sintaxe inválida.
      res
    ...
    res.status(422).send(`cannot update rota: ${err}`).end();
  }
};
...
const restore = async (req, res) => {
  try {
    ...
    if (id) {
      ...
      res.status(200).json('Rota reativada com sucesso.').end();
    } else {
      // 400 Bad Request Essa resposta significa que o servidor não entendeu a requisição pois está com uma sintaxe inválida.
      res
        .status(400)
    ...
    res.status(422).send(`Cannot restore route: ${err}`).end();
  }
};
...
const remove = async (req, res) => {
  try {
    ...
    if (id) {
      ...
      res.status(200).json('Rota deletada com sucesso.').end();
    } else {
      // 400 Bad Request Essa resposta significa que o servidor não entendeu a requisição pois está com uma sintaxe inválida.
      res
        .status(400)
    ...
    res.status(422).send(`Cannot delete route: ${err}`).end();
  }
};
